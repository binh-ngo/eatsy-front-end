...
function GalleryTile(props) {
  ...
  return (
    ...
    <Container fluid className="galleryTileCon">
      ...
      <a id="imgLink" href={`https://eatsy.herokuapp.com/users/${props.username}`} ><img className="galleryUserImg" src={props.companyUserImg}></img></a>
      <div id="chefInfoDiv">
        <a id="cardLink" href={`https://eatsy.herokuapp.com/users/${props.username}`} > <p className="chefName">{props.companyName}</p> </a>
        <div id="gallerTagBox">
        ...
        {props.companyMenu.map(item => {
          ...
          key={props.id}
          link={`https://eatsy.herokuapp.com/users/${props.username}`}
          name={item.name}
          src={item.src}
